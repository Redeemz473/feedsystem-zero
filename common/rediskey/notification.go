...
// 每次未读数可能发生变化时 INCR 一次：
//   - notification-job 新增未读通知 → INCR
//   - notification-job 撤回原本未读的通知 → INCR
//   - notification-rpc MarkNotificationRead 成功命中未读行 → INCR
//   - notification-rpc MarkAllNotificationsRead 有未读被批量置读 → INCR
//
// version key 本身不设 TTL：单条 int64 成本可忽略，且需要跨会话稳定递增。
// 格式: fsz:notification:unread:{userID}:version
...
// NotificationUnreadCountKey 用户当前版本下的未读数快照，value=int64 字符串。
// 读侧先取 version，再拼出这个 key 去 GET；miss 时走 MySQL COUNT 并回写。
// bump 版本号时同时 DEL 上一版本的 key，避免高频写入下 stale 缓存堆积到 TTL 才过期。
// 格式: fsz:notification:unread:{userID}:v:{version}
...
// 供 Lua 脚本按 "prefix .. tostring(version)" 拼出旧版本 key 进行清理，
// 保证 Go 侧与 Lua 侧使用同一套 key 命名规则，一处修改两处生效。
// 格式: fsz:notification:unread:{userID}:v:
