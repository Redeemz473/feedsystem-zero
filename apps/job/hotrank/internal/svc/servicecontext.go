...
func (s *ServiceContext) Close() {
	...
	if s.Consumer != nil {
		if err := s.Consumer.Close(); err != nil {
			logx.Errorf("close hotrank kafka consumer failed: %v", err)
		}
	}
	if s.RedisCli != nil {
		if err := s.RedisCli.Close(); err != nil {
			logx.Errorf("close hotrank redis client failed: %v", err)
		}
	}
	if s.GormDB == nil {
		return
	...
	if err != nil {
		logx.Errorf("get hotrank sql db failed: %v", err)
		return
	}
	if err := sqlDB.Close(); err != nil {
		logx.Errorf("close hotrank sql db failed: %v", err)
	}
}
