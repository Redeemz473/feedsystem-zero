...
func TestIsRetryableInteractionDBError(t *testing.T) {
	tests := []struct {
		name string
		err  error
		want bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := isRetryableInteractionDBError(tt.err); got != tt.want {
				t.Fatalf("isRetryableInteractionDBError() = %v, want %v", got, tt.want)
			}
		})
	}
}

func TestInteractionDBRetryDelayIsBounded(t *testing.T) {
	tests := []struct {
		retry int
		min   time.Duration
		max   time.Duration
	...
	for _, tt := range tests {
		delay := interactionDBRetryDelay(tt.retry)
		if delay < tt.min || delay > tt.max {
			t.Fatalf("interactionDBRetryDelay(%d) = %s, want [%s, %s]", tt.retry, delay, tt.min, tt.max)
		}
	}
}
