...
func (l *IsLikedLogic) IsLiked(in *interaction.IsLikedReq) (*interaction.IsLikedResp, error) {
	...
	// 2. 登录用户需要校验 video_id，并确认视频存在且未删除。
	...
	// 3. 优先查询 Redis LikeStateKey(video_id,user_id)。
	...
	// 4. Redis 未命中时查询 MySQL likes 表：video_id + user_id + status=1 + deleted_at IS NULL。
	...
	// 5. 将 MySQL 查询结果回写 Redis，减少详情页、列表页反复查库。
	if dbLiked {
